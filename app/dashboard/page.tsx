...
const Dashboard: React.FC = () => {
  ...
  const updateProgress = () => {
    ...
    const completionPercentage = calculateCompletion(chaptersData.length);
  ...
  const updateBadges = () => {
    ...
    if (isChapterCompleted("chapter-2")) earnedBadges.push("Process Pro");
    if (isChapterCompleted("chapter-3")) earnedBadges.push("Thread Theorist");
    if (isChapterCompleted("chapter-7")) earnedBadges.push("Memory Maven");
  ...
  const handleResumeClick = () => {
    const lastChapterId = getLastVisitedChapter();
    if (lastChapterId) {
